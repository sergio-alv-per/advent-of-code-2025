...
use winnow::combinator::{alt, fail, repeat, separated};
...
fn parse_grid(input: &mut &str) -> Result<Vec<Vec<Square>>> {
    separated(1.., parse_row, newline).parse_next(input)
}
...
fn roll_in_adjacent(
    ...
    return match (possible_adj_i, possible_adj_j) {
...
fn rolls_in_adjacent_squares(i: usize, j: usize, grid: &Vec<Vec<Square>>) -> i32 {
    let mut rolls = 0;
...
fn solve(input: &str) -> i32 {
    let grid = parse_input!(parse_grid, input);
    ...
    grid.iter()
        ...
        .filter_map(|(i, j, sq)| {
            if let Square::PaperRoll = sq {
                Some(rolls_in_adjacent_squares(i, j, &grid))
            } else {
                None
            }
        })
        .filter(|&n_rolls| n_rolls < 4)
        .count()
        .try_into()
        .unwrap()
}
...
mod tests {
    ...
    test_with_files!("04", solve, 13, 1480);
}
